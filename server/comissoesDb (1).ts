...
export async function listarInadimplentes(mes?: number, ano?: number, status?: string, busca?: string, formaPagamento?: string) {
  ...
  if (status) {
    if (status === 'PENDENTE') {
      // PENDENTE engloba: NULL, vazio, 'PENDENTE' e 'Não trabalhada' (legado da base original)
      whereClause.push("(i.status IS NULL OR i.status = '' OR i.status = 'PENDENTE' OR i.status = 'Não trabalhada')");
    } else {
